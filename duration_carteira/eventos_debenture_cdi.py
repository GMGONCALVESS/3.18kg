...
def get_closest_tenor(days, tenores_dict):
    ...
    closest_tenor = min(tenores_dict, key=lambda k: abs(tenores_dict[k] - days))
...
def calcular_juros_acimadi(vne, spread_anual, dp, start_date, end_date):
    ...
    tdi_k_diario = indicador_cdi[(indicador_cdi['index'] >= start_date) & (indicador_cdi['index'] < end_date)]

    tdi_k_diario.to_excel('tdi_k_diario.xlsx')
    
    # Calcular a taxa DI diária
    tdi_k_diario.loc[:, 'tdi_k_diario'] = (1 + tdi_k_diario['px_last']/100) ** (1/dias_uteis_ano) - 1
...
posicoes_query = pd.read_sql('SELECT * FROM posicoes_pbi ORDER BY posicao_id', engine)

# convert the SQL table into a pandas dataframe
posicoes_pbi = pd.DataFrame(posicoes_query)

# sql query to read all the records
eventos_query = pd.read_sql('SELECT * FROM eventos_debenture ORDER BY deb_id', engine)

# convert the SQL table into a pandas dataframe
eventos_debenture = pd.DataFrame(eventos_query)

# sql query to read all the records
eventos_cricra_query = pd.read_sql('SELECT * FROM eventos_cricra ORDER BY cricra_id', engine)
...
eventos_debenture_cdi.loc[eventos_debenture_cdi['evento'].isin(['Vencimento (resgate)', 'Resgate total antecipado']), 'percentual_taxa'] = 100
...
for index, row in eventos_debenture_cdi.iterrows():
    if previous_codigo is not None and previous_codigo != row['codigo_ticker']:
        ...
        previous_pu = row['vne']  # Reset previous_pu to 1000.0 for the new ticker
    ...
    previous_pu = eventos_debenture_cdi.at[index, 'pu']  # Store the updated 'pu'


indicador_brasil_usa_query = pd.read_sql("SELECT * FROM indicadores_brasil_usa WHERE pais = 'Brasil' ORDER BY indicador_id", engine)

indicador_brasil = pd.DataFrame(indicador_brasil_usa_query)

# sql query to read all the records
indicadorcdi_query = pd.read_sql('SELECT * FROM indicador_cdi ORDER BY index', engine)
...
eventos_debenture_cdi['data_evento'] = pd.to_datetime(eventos_debenture_cdi['data_evento'])
...
eventos_debenture_cdi = eventos_debenture_cdi.rename(columns={'px_last': 'cdi'})
...
custom_business_day = CustomBusinessDay(calendar=CustomBusinessCalendar())
new_dates = pd.bdate_range(start = min_date, end = max_date, freq = custom_business_day)
...
indicador_cdi['tenor'] = indicador_cdi['days'].apply(lambda x: get_closest_tenor(x, tenores))

indicador_cdi = indicador_cdi.merge(curva_di[['tenor', 'bid_yield']], on='tenor', how='left')

indicador_cdi['px_last'] = indicador_cdi['px_last'].fillna(indicador_cdi['bid_yield'])
indicador_cdi = indicador_cdi.drop(columns=['bid_yield'])

eventos_debenture_cdi['days'] = (eventos_debenture_cdi['data_evento'] - datetime.today()).dt.days

# Step 2: Apply the function to create a new 'tenor' column
eventos_debenture_cdi['tenor'] = eventos_debenture_cdi['days'].apply(lambda x: get_closest_tenor(x, tenores))

# First, perform the merge based on the 'tenor' column
eventos_debenture_cdi = eventos_debenture_cdi.merge(curva_di[['tenor', 'bid_yield']], on='tenor', how='left')

eventos_debenture_cdi['cdi'] = eventos_debenture_cdi['cdi'].fillna(eventos_debenture_cdi['bid_yield'])
...
eventos_debenture_cdi['start_date'] = eventos_debenture_cdi['data_evento'].shift(1)
eventos_debenture_cdi['end_date'] = eventos_debenture_cdi['data_evento']

eventos_debenture_cdi.loc[eventos_debenture_cdi['start_date'].isnull(), 'start_date'] = eventos_debenture_cdi['inicio_rentabilidade']

# Create a boolean Series where the current 'codigo_ticker' is different from the previous one
codigo_change = eventos_debenture_cdi['codigo_ticker'] != eventos_debenture_cdi['codigo_ticker'].shift(1)

# Use .loc to update 'start_date' where 'codigo_ticker' changes
eventos_debenture_cdi.loc[codigo_change, 'start_date'] = eventos_debenture_cdi['inicio_rentabilidade']
...
for i in range(1, len(eventos_debenture_cdi)):  # Start from 1 to avoid accessing index -1
    ...
    if ((eventos_debenture_cdi.loc[i, 'start_date'] == eventos_debenture_cdi.loc[i, 'end_date']) & (eventos_debenture_cdi.loc[i, 'evento'] == 'Pagamento de juros')) or (eventos_debenture_cdi.loc[i - 1, 'evento'] == 'Premio'):
        ...
        eventos_debenture_cdi.loc[i, 'start_date'] = eventos_debenture_cdi.loc[i - 1, 'start_date']

# Now, apply the get_busdays function using the start_date and end_date
eventos_debenture_cdi['dp'] = eventos_debenture_cdi.apply(
    lambda row: get_busdays(row['start_date'], row['end_date']) if pd.notnull(row['start_date']) else None,
    axis=1
)
    ...
    if eventos_debenture_cdi.loc[i, 'evento'] == 'Amortizacao':
        if eventos_debenture_cdi.loc[i, 'codigo_ticker'] == 'SNGO18':
            ...
            eventos_debenture_cdi.loc[i, 'juros_pagos'] = (
                eventos_debenture_cdi.loc[i - 1, 'vne'] * (eventos_debenture_cdi.loc[i, 'percentual_taxa'] / 100)
            )
        else:
            # Update 'juros_pagos' for 'Amortizacao' event using the previous row 'pu'
            eventos_debenture_cdi.loc[i, 'juros_pagos'] = (
                eventos_debenture_cdi.loc[i - 1, 'pu'] * (eventos_debenture_cdi.loc[i, 'percentual_taxa'] / 100)
            )
    elif eventos_debenture_cdi.loc[i, 'evento'] == 'Premio':
        eventos_debenture_cdi.loc[i, 'juros_pagos'] = eventos_debenture_cdi.loc[i, 'valor_pago']
...
mask = eventos_debenture_cdi['juros_pagos'].isna()  # Create a mask for rows where 'juros_pagos' is null
eventos_debenture_cdi.loc[mask, 'juros_pagos'] = eventos_debenture_cdi[mask].apply(apply_calcular_juros, axis=1)
...
eventos_debenture_cdi['month_w'] = eventos_debenture_cdi['data_evento'].dt.strftime('%B').map(month_mapping)
...
current_entries = grouped_df[['codigo_custodia_ticker', 'fundo', 'quantidade']][grouped_df['data'] == min_date].reset_index(drop=True)
...
new_entries = pd.merge(new_entries, current_entries, on='key').drop('key', axis=1)

new_entries = new_entries.drop(columns=['codigo_custodia_ticker_x', 'fundo_x', 'quantidade_x'])
...
eventos_debenture_cdi = eventos_debenture_cdi.merge(
    grouped_df,
    left_on=['data_liquidacao', 'codigo_ticker'],  # Columns from eventos_debenture_cdi
...
eventos_debenture_cdi = eventos_debenture_cdi.drop(columns=['data', 'codigo_custodia_ticker'])

eventos_debenture_cdi = eventos_debenture_cdi[eventos_debenture_cdi['data_evento']>='01-01-2024']

eventos_debenture_cdi['quantidade'] = eventos_debenture_cdi['quantidade'].fillna(method='ffill')
eventos_debenture_cdi['fundo'] = eventos_debenture_cdi['fundo'].fillna(method='ffill')
...
eventos_debenture_cdi['valor_recebido'] = np.where(
    ...
    eventos_debenture_cdi['valor_pago'] * eventos_debenture_cdi['quantidade'],  # If 'valor_pago' is not null
    eventos_debenture_cdi['juros_pagos'] * eventos_debenture_cdi['quantidade']  # If 'valor_pago' is null
)
...
table_eventos_debenture_cdi = Table('eventos_debenture_cdi', metadata, autoload_with=engine)
...
eventos_debenture_cdi.to_sql(
    name="eventos_debenture_cdi", # table name
    con=engine,  # engine
